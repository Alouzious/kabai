...
def create_category(
    category_in: CategoryCreate, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
...
def delete_category(
    category_id: uuid.UUID, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
