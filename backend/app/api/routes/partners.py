...
def create_partner(
    partner_in: PartnerCreate, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
...
def delete_partner(
    partner_id: uuid.UUID, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
